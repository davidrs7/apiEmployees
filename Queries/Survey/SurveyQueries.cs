...
namespace Api.Queries
{
    public class SurveyQueries
    {
        ...
        public string HeadersByUser { get; } = "SELECT DISTINCT SH.Id, S.Id AS SurveyId, SULR.UserId, S.Available, S.Name, S.Description, SH.Started, SH.Finished, SH.Title, IF(SR.Id IS NULL, 0, 1) AS IsAnswered, IF(SR.Id IS NULL, 0, SR.Draft) AS Draft FROM Survey S INNER JOIN Survey_Header SH ON S.Id = SH.SurveyId INNER JOIN Survey_User_Login_Rel SULR ON (SH.Id = SULR.SurveyHeaderId AND SULR.Active = 1) LEFT JOIN Survey_Response SR ON SR.SurveyUserId = SULR.Id WHERE SH.Started <= NOW() AND SH.Finished >= NOW() AND SULR.UserId = @UserId";
